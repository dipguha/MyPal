...
- Stepper adapts to role and plan — Owner/Admin on Family plan see 5 steps; all other members see 4 steps
...
- Profile setup (display name, emoji avatar) — display name is captured at sign-up; avatar is set in My Account → My Profile. Onboarding has no Profile step (kept deliberately fast).
...
|---------|-------------|-------------------------------|
| New member (Individual — Owner) | Just signed up for an Individual plan | Set up briefing and interests quickly |
| New member (Family — Owner) | Just signed up for a Family plan | Invite family members, configure own briefing and interests |
| New member (Family — Admin) | Accepted an invitation; first sign-in | Optionally add further family members, configure own briefing and interests |
| New member (Adult Member / Grand Parent) | Accepted an invitation; first sign-in | Configure own briefing and interests |
| New member (Teenager) | Accepted an invitation; first sign-in | Configure own briefing and interests |
| New member (Child) | Profile created by Owner; first sign-in | Configure own briefing and interests (same journey as Teenager) |
...
- Owner or Admin on Family plan: Welcome → Family → Briefing → Interests → Done (5 steps)
- All other members (Owner on Individual plan, Adult Member, Grand Parent, Teenager, Child): Welcome → Briefing → Interests → Done (4 steps)

There is no Profile step — display name is set at sign-up and avatar in My Account → My Profile. The Child journey is identical to Teenager.
...
|---|-------------|----------|-----------|
| F-01 | Onboarding stepper adapts to role and plan: Owner/Admin on Family plan see 5 steps (incl. Family); all other members — Owner on Individual plan, Adult Member, Grand Parent, Teenager and Child — see 4 steps. There is no Profile step | P0 | Stepper — Owner/Admin Family plan; Stepper — Owner Individual plan; Stepper — non-Owner member; Stepper — Child same as Teenager |
| F-02 | *Retired (v1.3).* Profile setup removed from onboarding — display name is set at sign-up and avatar in My Account → My Profile. | — | — |
...
| F-05 | Daily Briefing step is shown to all roles (incl. Child); collects home postcode, work address, commute mode, and news topics; step is skippable | P0 | Briefing step — save; Briefing step — skip |
| F-06 | Interests step is shown to all roles (incl. Child); displays a tag cloud; members can select up to 5 tags; further taps ignored until a tag is deselected; step is skippable | P1 | Interests step — select tags; Interests step — cap at 5; Interests step — skip |
...
Feature: Onboarding stepper renders correct steps based on role and plan
  ...
  Scenario Outline: Owner or Admin on Family plan sees 5-step stepper
    Given the member's role is "<role>" and the account plan is "Family"
    When the member lands on the Onboarding flow
    Then the stepper shows exactly 5 steps: Welcome, Family, Briefing, Interests, Done
  ...
  Scenario: Owner on Individual plan sees 4-step stepper
    Given the member's role is "Owner" and the account plan is "Individual"
    When the member lands on the Onboarding flow
    Then the stepper shows exactly 4 steps: Welcome, Briefing, Interests, Done
    And no Family step is present

  Scenario Outline: All other members see the 4-step stepper (no Family step)
...
# F-02: Retired in v1.3 — Profile step removed from onboarding (display name set
# at sign-up; avatar set in My Account → My Profile). No scenarios.
...
Feature: Daily Briefing step is shown to all roles
...
# F-06: Interests step
Feature: Interests step is shown to all roles; capped at 5 tags
...
Feature: Done screen shows a contextual completion summary
  Scenario: Done screen — all completed (Family plan Owner)
    Given an Owner on a Family plan completed the Family, Briefing, and Interests steps
  ...
  Scenario: Done screen — partial completion (Individual plan, briefing and interests skipped)
    Given an Owner on an Individual plan skipped the Briefing and Interests steps
    When they reach the Done screen
    Then the Briefing row shows a muted dash and "set up any time in My Account"
    And the Interests row shows a muted dash and "set up any time in My Account"
    And no Family row is shown

  Scenario: Done screen — Child (same as Teenager)
    Given a member with role "Child" completed the Briefing and Interests steps
    When they reach the Done screen
    Then the Briefing and Interests rows each show a green checkmark
    And no Family row is shown
...
Feature: onboarding_complete is set server-side on "Enter MyPal"
  Scenario: Onboarding complete — flag set server-side
    ...
    Then a PATCH to /api/v1/onboarding/complete sets onboarding_complete = true on the member's record
...
Feature: Onboarding state is preserved if member abandons mid-flow
  Scenario: Mid-flow abandonment — state preserved
    Given a member has completed the Briefing step and is on the Interests step
    ...
    And data entered on the Briefing step is preserved
```
...
| US-02 | New member (Family Owner or Admin) | Add family members and assign their roles | Everyone has the right level of access when they join |
| US-03 | New member (Family Owner or Admin) | Send invitation emails during onboarding | My family can join without me having to find a separate invite option |
| US-04 | New member (all roles) | Set up my own daily briefing with my commute and news preferences | My first morning summary is relevant to my life |
| US-05 | New member (all roles) | Select my own interests | My content feed is personalised to me from day one |
...
| NF-04 | Privacy enforcement | Access rules enforced server-side — never frontend-only. Every member submits only their own onboarding data; the family-members endpoint is restricted to Owner and Admin on a Family plan |
...
Feature: Individual plan Owner onboarding end-to-end
  Scenario: Happy path — Individual plan Owner
    ...
    Then the Welcome screen previews the Briefing and Interests steps
    ...
    Then the Done screen shows green checkmarks for Briefing and Interests
    When they click "Enter MyPal"
...
Feature: Family plan Owner onboarding end-to-end
  Scenario: Happy path — Family plan Owner with members added
    ...
    And add 2 Adult Members and 1 Child (no email) on the Family step and click "Continue"
    ...
    Then the Done screen shows: Family (3 members added) ✓, Briefing ✓, Interests ✓
    When they click "Enter MyPal"
...
Feature: Invited member onboarding end-to-end
  Scenario Outline: Happy path — invited member sets up briefing and interests
    Given a new member with role "<role>" has accepted an invitation and signs in for the first time
    When they land on Onboarding
    Then the stepper shows 4 steps: Welcome, Briefing, Interests, Done
    When they complete the Briefing step
...
Feature: Child member onboarding end-to-end
  Scenario: Happy path — Child journey is identical to Teenager
...
Feature: Member skips optional steps during onboarding
  Scenario: Owner skips Briefing and Interests
    Given an Owner on an Individual plan has landed on Onboarding
    When they click "Skip for now" on the Briefing step
    And click "Skip — personalise later" on the Interests step
    Then the Done screen shows: Briefing — (set up in My Account), Interests — (set up in My Account)
    When they click "Enter MyPal"
  ...
  Scenario: Family plan Owner skips Family step
    Given an Owner on a Family plan is on the Family step
...
Feature: Onboarding edge cases and error handling
  ...
  Scenario: Back navigation preserves data
    ...
    Then they return to the Briefing step
    And the postcode, commute mode, and news topics they entered are still populated

  Scenario: Browser closed mid-onboarding — re-entry at last completed step
    Given a member has completed the Briefing step and closed the browser
    When they sign in again
    Then they are routed to Onboarding at the step after Briefing
    And their Briefing data is preserved
```
...
|-------|-------------|-----------|-------|
| `members.display_name` | Member's display name | Yes | Set at sign-up from first_name; not collected in onboarding (edit in My Account → My Profile). For added family members it is captured on the Family step |
| `members.role` | Role assigned to a family member | Yes (per added member) | Enum (canonical codes): `owner`, `admin`, `adult`, `grandparent`, `teenager`, `child` |
...
| `members.onboarding_complete` | Whether onboarding has been finished | System | Defaults to `false`; set to `true` server-side on "Enter MyPal" click. Read by `auth#me` to drive post-sign-in routing |

**Note:** Avatar (`members.avatar_emoji`) is **not** set during onboarding — it is captured in My Account → My Profile. This field is therefore owned by the My Profile spec, not this one.
...
**Welcome step:** Large emoji (🎉), Playfair Display heading, short subtitle, preview rows showing the steps relevant to this member's role. Single "Let's go" CTA. No skip option on this step. There is no Profile step — display name is set at sign-up and avatar in My Account → My Profile.
...
**Daily Briefing step (all roles):** Home postcode and work address (text inputs), commute mode (4-option pill selector: Drive / Transit / Cycle / Walk), news topics (multi-select tags). "Skip for now" available.

**Interests step (all roles):** Full tag cloud with toggle behaviour. Count shown below ("N selected"). "Skip — personalise later" available.
...
| Interests setup rate | > 55% | Analytics: Interests step "Finish setup" vs. "Skip" |
...
- [ ] Access rules enforced server-side for all roles — the family-members endpoint restricted to Owner/Admin on a Family plan; every member submits only their own onboarding data
- [ ] Stepper renders 4 steps for members without family management and 5 for Owner/Admin on Family plan; Individual plan never sees the Family step; there is no Profile step
- [ ] Child onboarding journey is identical to Teenager (Welcome → Briefing → Interests → Done)
- [ ] Child added-member email is optional — an invitation is sent only when an email is provided
...
| 1.3 | 2026-06-15 10:51 | Claude Code | Removed the Profile step from onboarding — display name is set at sign-up, avatar in My Account → My Profile (kept onboarding fast). Child journey now mirrors Teenager (Child sees Briefing + Interests). Stepper counts updated to 5 (Owner/Admin on Family) / 4 (all other members). F-02 retired; "Child does not see Briefing/Interests" rules removed. NF-04 and §12 access wording corrected (every member onboards; only the family-members endpoint is Owner/Admin-gated). §8 role enum corrected to canonical codes (owner/admin/adult/grandparent/teenager/child); `onboarding_complete` corrected from `users` to `members` (§8 + F-09) to match the live schema and `auth#me`. Updated §2/§3/§4/§5a/§5b/§5.1/§6/§7/§8/§9/§12. |
