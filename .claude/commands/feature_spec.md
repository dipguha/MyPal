...
- You must use feature_spec_template.md file as reference in the reference folder.
...
Create a markdown spec document that Plan mode can use directly and save it in the _specs folder using the `feature_slug`. Use the exact structure as defined in the spec template file here: @references/feature_spec_template.md. Do not add technical implementation details such as code examples. Try to limit the feature to less than 10 (the most important ones) unless not possible and create user stories for all of them.
