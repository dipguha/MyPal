...
/* Key Dates — Others section seed data (module-level so Add/Edit handlers share one source) */
...
function LifeAdminScreen() {
  ...
  const [othersRoutinesState, setOthersRoutinesState] = useState(OTHERS_INITIAL_KD);
  ...
  const views = {
    ...
    "Key Dates": (()=>{
      ...
      // ── Others routines (state initialised from module-level OTHERS_INITIAL_KD) ──
      const OTHERS = othersRoutinesState;
                ...
                {OTHERS.map((r,i,arr)=>(
                  <div key={r.id}
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}} onClick={e=>e.stopPropagation()}>
                      ...
                      {r.nextDue && kdSep}
                      {(()=>{const st=KD_STATUS[r.status]||KD_STATUS.ok;return <span style={{...badge(st.bg,st.color)}}>{st.label}</span>;})()}
                      {kdSep}
                      <button className="btn-sm" style={{fontSize:10,padding:"2px 7px",color:T.rose,borderColor:T.rose+"44"}}
                        onClick={e=>{e.stopPropagation();setOthersRoutinesState(prev=>prev.filter(o=>o.id!==r.id));}}>Delete</button>
                    </div>
                  </div>
                ))}
    ...
    <div>
      ...
      {addOthersOpen && (()=>{
        ...
        const handleAdd = () => {
          ...
          setOthersRoutinesState(prev=>[...prev, newRoutine]);
          close();
        };
        return (
          <>
      ...
      {othersEditItem && (()=>{
        ...
        const handleSave = () => {
          ...
          setOthersRoutinesState(prev=>prev.map(o=>o.id===r.id?updated:o));
          close();
        };
        return (
          <>
