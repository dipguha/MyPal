...
Scaffolded (Rails 7.2, API-only) with the sign-up/auth, identity, and onboarding slices implemented; other feature modules are pending. Layout (per `backend/CLAUDE.md`): thin controllers under `app/controllers/api/v1/` → fat services in `app/services/` → ActiveRecord models; **Pundit** policies in `app/policies/` (`pundit_user` = `current_member`); **Blueprinter** serializers; JWT verification in `app/services/cognito_jwt_verifier.rb`. Cognito sign-up lives in `CognitoService` — which also sends family-member invitation emails via `AdminCreateUser` during onboarding; the account-creation transaction in `AccountBootstrapService`; onboarding (role/plan step set, briefing/interests, family invitations) in `OnboardingStatusService` + `OnboardingService` + `FamilyMemberInvitationService`. All endpoints are versioned under `/api/v1/` (`auth/sign-up|confirm|resend-verification|me`, `onboarding/status|family_members|briefing|interests|complete`, plus `/healthz`). Tests are RSpec request/service/model specs. See ADR-015 and §4.
...
**Migrations** live in `backend/db/migrate/`. The baseline migration creates 7 identity tables (`member_roles`, `role_permissions`, `accounts`, `users`, `members`, `account_settings`, `member_settings`); subsequent migrations reseed `member_roles` to the canonical set, add `groups`/`group_members`, add `users.marketing_opt_in` + `members.onboarding_complete`, and add the onboarding briefing/interests columns to `member_settings` (`home_postcode`, `work_address`, `commute_mode`, `news_topics[]`, `interests[]`). Extensions: `citext`, `pgcrypto`. Feature tables (finance, health, recipes, tasks, journal, etc.) are added by their own migrations.
...
| 1.5 | Claude Code | 15/06/2026 11:45 UTC | Onboarding implementation: §2 backend now lists the `/api/v1/onboarding/*` endpoints and the onboarding services; noted `CognitoService.AdminCreateUser` as the family-invitation email touchpoint; §5 records the new `member_settings` briefing/interests columns. |
